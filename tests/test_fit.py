...
def test_dataset1():
    ...
    assert np.isclose(80.36314052579074, sigmet.fit(start_date, end_date, force_start=True), atol=0.01)
    assert np.isclose(80.36314052579074, sigmet.fit(dates_dataset1[2], end_date), atol=0.01)
...
def test_dataset2():
    ...
    assert np.isclose(363.29172903368146, sigmet.fit(start_date, end_date, recovery_threshold=0.85), atol=0.01)
...
def test_dataset3():
    ...
    assert np.isclose(45.77548478066123, sigmet.fit(start_date, end_date, recovery_threshold=1), atol=0.01)
